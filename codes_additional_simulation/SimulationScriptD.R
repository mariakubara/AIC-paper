...
# Uniform 'agriculture' 50%, uniform 'construction' 25%, clusters per 25 obs each 'service' 25%

numObs<-list(c(100, 50, 2, 25),c(250, 125, 5, 25),c(500, 250, 10, 25))
...
for(n in 1:3){
  for(s in 1:20){
    ...
    ################# Generating random points from uniform distribution v2 ###################
    uniform7 <- runifpoint(numObs[[n]][2], win=W, giveup=1000, nsim=1, drop=TRUE, ex=NULL)
    
    #extracting the points 
    unif.cords <- coords(uniform7)
    unif.cords.sp<-SpatialPoints(unif.cords)
    ...
    # creating a data frame with simulated companies characteristics
    data2<-cbind(unif.cords.sp@coords, powLub)
    ...
    data3 <- runifpoint(numObs[[n]][3], win=W, giveup=1000, nsim=1, drop=TRUE, ex=NULL)
    ...
    # saving the results
    write.table(resultsMat, file="results_additional/simulationTwoUniformCluster25.csv", row.names = F, col.names = T)
